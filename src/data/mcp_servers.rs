...
impl McpServersConfig {
    ...
    pub fn create_sample() -> Self {
        ...
        config.add_server(
            ...
            McpServer::stdio("uvx".to_string(), vec!["imagesorcery-mcp".to_string()])
                .with_env(env)
                .with_enabled(false),
        );
        ...
        config.add_server(
            ...
            McpServer::stdio("node".to_string(), vec!["server.js".to_string()])
                .with_working_directory("/path/to/mcp/server".to_string())
                .with_enabled(false),
        );
...
mod tests {
    ...
    fn test_stdio_server_creation() {
        ...
        assert_eq!(
            server.working_directory.as_ref().unwrap(),
            "/path/to/server"
        );
    }
    ...
    fn test_claude_format_compatibility() {
        ...
        assert_eq!(
            github_server.url.as_ref().unwrap(),
